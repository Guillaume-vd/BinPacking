...
public class ProgramationLinéaire {
    ...
    public ProgramationLinéaire(Info info){
        ...
        int maxNbBins = info.getData().size();
        String lstr = "";
        ...
        //System.out.println("Build variable...");
        MPVariable[][] x = new MPVariable[nbItems][maxNbBins];
        for (int i = 0; i < nbItems; i++){
            for (int j = 0; j < maxNbBins; j++){
                lstr = "X[" + i + "," + j + "]";
                x[i][j] = lSolver.makeBoolVar(lstr);
            }
        }
        MPVariable[] y = new MPVariable[maxNbBins];
        for (int j = 0; j < maxNbBins; j++){
            lstr = "Y" + j;
            y[j] = lSolver.makeBoolVar(lstr);
        }
        //System.out.println("#" + lSolver.numVariables());

        //Création des contraintes
        //System.out.println("Build constraints...");
        ...
        MPConstraint[] lConstraint2 = new MPConstraint[maxNbBins];
        for (int j = 0; j < maxNbBins; j++){
        ...
        //System.out.println("#" + lSolver.numConstraints());
        ...
        //System.out.println("PL is running...\n");
